...
function flow(flowName,states){
        ...
        function registerState(state){
            ...
            function wrapUpdates(stateName){
                return function() {
                    ...
                    return toBeExecuted.why(decideMotivation(self,undefined,state)).apply(self,mkArgs(arguments,0));
                }
            }
            ...
            self[state] = addErrorTreatment(wrapUpdates(state));
        }
        ...
        function updateJoinsAfterCall(stateName){
            ...
            statesRegister[stateName].joins.forEach(function (joinName) {
                ...
                if(joinsRegister[joinName]['tryOnNextTick'] === false) {
                    ...
                    var caller = whys.getGlobalCurrentContext().currentRunningItem;
                    process.nextTick(function(){
                        tryRunningJoin(joinName,caller);
                    })
                }
            });


            function tryRunningJoin(joinName,caller){
                ...
                if(joinReady(joinName)){
                    var toRun = runJoin.why(
                        decideMotivation(self,joinName,joinName),
                        caller
                    );
    ...
    flowStatus = "running";
    this.begin.apply(this, mkArgs(arguments, 0));
    return this;
}
...
function decideMotivation(flow, motivation, stateName){
    if(motivation === undefined){
        if(flow.getCurrentPhase()){
            ...
            motivation = "***Starting flow: " + flow.getName();
        }
    }
    return motivation;
}
